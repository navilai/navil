# Adversarial QA Test Suite Implementation Plan

> **For agentic workers:** REQUIRED: Use superpowers:subagent-driven-development (if subagents available) or superpowers:executing-plans to implement this plan. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Build a red-team adversarial test suite in `tests/test_adversarial.py` that attempts to break every Navil security control and surfaces real bugs as pytest failures.

**Architecture:** Single test file, 8 attack classes, two execution layers: Layer 1 (direct Python class instantiation) and Layer 2 (ASGI via `httpx.AsyncClient + ASGITransport`). Tests are written to EXPECT attacks to fail — a pytest failure = a confirmed security finding.

**Tech Stack:** pytest, pytest-asyncio, httpx (ASGITransport), PyJWT, unittest.mock (AsyncMock), threading, Python pathlib

---

## Pre-flight

Confirm baseline passes before writing any tests:
...
## Chunk 1: File Skeleton + Auth Tests

### Task 1: Create test file with shared fixtures

**Files:**
- Create: `tests/test_adversarial.py`

- [ ] **Step 1: Create the file with imports and shared fixtures**

Write `tests/test_adversarial.py` with this exact content:
...
```bash
cd /Users/clawbot/claude/navil && python -c "import tests.test_adversarial" 2>&1
```
...
git commit -m "test(adversarial): add file skeleton and shared fixtures"
```

---

### Task 2: TestAuthBypass — 9 attack vectors

**Files:**
- Modify: `tests/test_adversarial.py` (append class)

- [ ] **Step 1: Append `TestAuthBypass` class to the file**
...
```bash
cd /Users/clawbot/claude/navil && pytest tests/test_adversarial.py::TestAuthBypass -v 2>&1 | tail -20
```

Expected: All 9 pass. If `test_forged_x_agent_name_no_sanitization` fails, it means validation was added — update the assertion accordingly.

- [ ] **Step 3: Commit**

```bash
git add tests/test_adversarial.py
git commit -m "test(adversarial): add TestAuthBypass — 9 auth bypass attack vectors"
```

---

### Task 3: TestJWTSecurity — 9 JWT attack vectors

**Files:**
- Modify: `tests/test_adversarial.py` (append class)

- [ ] **Step 1: Append `TestJWTSecurity` class**
...
```bash
cd /Users/clawbot/claude/navil && pytest tests/test_adversarial.py::TestJWTSecurity -v 2>&1 | tail -20
```

Expected:
- `test_issued_tokens_fail_jwt_decode` — PASS (the test asserts an exception IS raised — confirms High finding)
- `test_missing_exp_accepted_as_eternal` — **PASS is the finding** (token accepted without expiry — eternal token confirmed; a pytest FAIL here would mean the bug doesn't exist)
- All others: PASS
...
```bash
git add tests/test_adversarial.py
git commit -m "test(adversarial): add TestJWTSecurity — 9 JWT attack vectors"
```

---

## Chunk 2: Rate Limit, Input Validation, JSON-RPC

### Task 4: TestRateLimitBypass — 4 vectors

**Files:**
- Modify: `tests/test_adversarial.py` (append class)

- [ ] **Step 1: Append `TestRateLimitBypass` class**
...
```bash
cd /Users/clawbot/claude/navil && pytest tests/test_adversarial.py::TestRateLimitBypass -v 2>&1 | tail -20
```

Expected:
- `test_key_collision_different_agents` — **FAIL is a confirmed Medium finding**
- All others: PASS
...
```bash
git add tests/test_adversarial.py
git commit -m "test(adversarial): add TestRateLimitBypass — 4 vectors including key collision finding"
```

---

### Task 5: TestInputValidation — 12 Pydantic boundary tests

**Files:**
- Modify: `tests/test_adversarial.py` (append class)

- [ ] **Step 1: Append `TestInputValidation` class**
...
```bash
cd /Users/clawbot/claude/navil && pytest tests/test_adversarial.py::TestInputValidation -v 2>&1 | tail -20
```

Expected:
- `test_null_byte_passes_pydantic` — **FAIL (pytest.skip) if finding confirmed**
- `test_newline_passes_pydantic` — **FAIL (pytest.skip) if finding confirmed**
- All others: PASS
...
```bash
git add tests/test_adversarial.py
git commit -m "test(adversarial): add TestInputValidation — 12 Pydantic boundary vectors"
```

---

### Task 6: TestJSONRPCAbuse — 10 JSON-RPC attack vectors

**Files:**
- Modify: `tests/test_adversarial.py` (append class)

- [ ] **Step 1: Append `TestJSONRPCAbuse` class**
...
```bash
cd /Users/clawbot/claude/navil && pytest tests/test_adversarial.py::TestJSONRPCAbuse -v 2>&1 | tail -20
```
...
```bash
git add tests/test_adversarial.py
git commit -m "test(adversarial): add TestJSONRPCAbuse — 10 JSON-RPC attack vectors"
```

---

## Chunk 3: ASGI Tests + Novel Findings

### Task 7: TestPathTraversal — 5 ASGI vectors

**Files:**
- Modify: `tests/test_adversarial.py` (append class)

- [ ] **Step 1: Append `TestPathTraversal` class**
...
```bash
cd /Users/clawbot/claude/navil && pytest tests/test_adversarial.py::TestPathTraversal -v 2>&1 | tail -20
```
...
```bash
git add tests/test_adversarial.py
git commit -m "test(adversarial): add TestPathTraversal — 5 ASGI path traversal vectors"
```

---

### Task 8: TestCORSBehavior — 4 ASGI CORS vectors

**Files:**
- Modify: `tests/test_adversarial.py` (append class)

- [ ] **Step 1: Append `TestCORSBehavior` class**
...
```bash
cd /Users/clawbot/claude/navil && pytest tests/test_adversarial.py::TestCORSBehavior -v 2>&1 | tail -20
```
...
```bash
git add tests/test_adversarial.py
git commit -m "test(adversarial): add TestCORSBehavior — 4 ASGI CORS attack vectors"
```

---

### Task 9: TestNewVulnerabilities — 5 novel findings

**Files:**
- Modify: `tests/test_adversarial.py` (append class)

- [ ] **Step 1: Append `TestNewVulnerabilities` class**
...
class TestNewVulnerabilities:
    ...
    def test_rate_limit_key_collision_confirmed(self) -> None:
        ...
        engine.policy = {
            "agents": {
                "a:b": {"allowed_tools": ["*"], "rate_limit_per_hour": 2},
                "a": {"allowed_tools": ["*"], "rate_limit_per_hour": 100},
            }
        }
    ...
    def test_jwt_expiry_not_enforced_via_proxy_hmac_path(self) -> None:
        ...
        token_id = cred_info["token_id"]

        # The token is in cm.credentials as ACTIVE
        # Wait would normally expire it, but the proxy never checks JWT exp
        # because verify_credential() always raises first
        ...
        # Even without sleeping, authenticate using the real proxy path
...
```bash
cd /Users/clawbot/claude/navil && pytest tests/test_adversarial.py::TestNewVulnerabilities -v 2>&1 | tail -20
```

Expected:
- `test_rate_limit_key_collision_confirmed` — **FAIL = Medium finding confirmed**
- `test_x_agent_name_no_length_limit` — **FAIL = Low finding confirmed**
- `test_x_agent_name_newline_log_injection` — **FAIL = Low finding confirmed**
- `test_jwt_expiry_not_enforced_via_proxy_hmac_path` — **FAIL = High finding confirmed**
- `test_anonymous_identity_via_x_agent_name` — PASS

- [ ] **Step 3: Commit**

```bash
git add tests/test_adversarial.py
git commit -m "test(adversarial): add TestNewVulnerabilities — 5 novel finding vectors"
```
...
- [ ] **Run the full adversarial suite and capture findings**

```bash
cd /Users/clawbot/claude/navil && pytest tests/test_adversarial.py -v 2>&1
```

Record every test that FAILS — each is a confirmed security finding.

- [ ] **Confirm no regressions in the rest of the suite**

```bash
pytest tests/ -q --ignore=tests/test_adversarial.py 2>&1 | tail -5
```
...
```bash
ruff check tests/test_adversarial.py
```
...
```bash
git add tests/test_adversarial.py
git commit -m "test(adversarial): final cleanup"
```

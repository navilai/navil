          ...
          {severities.map(s => (
            ...
            >
              ...
              {alertCounts[s] !== undefined && (
                <span className="bg-surface text-ink-secondary text-[10px] px-1.5 py-0.5 rounded-full min-w-[20px] text-center font-semibold">
                  {alertCounts[s]}
                </span>
              )}
            </button>
      ...
      {alerts.length === 0 ? (
        <div className="text-center py-16 animate-fadeIn">
          <div className="inline-flex items-center justify-center w-16 h-16 rounded-2xl bg-signal-green/10 border border-signal-green/20 mb-4">
            <Icon name="check" size={32} className="text-signal-green" />
          </div>
          <p className="text-ink-secondary">No alerts{filter ? ` with severity ${filter}` : ''}. Looking good!</p>
        </div>
      ) : (
          ...
          {alerts.map((alert, i) => (
            ...
            >
              ...
              >
                ...
                <span className="font-mono text-xs text-ink-secondary w-40 shrink-0">{alert.anomaly_type}</span>
                <span className="text-sm text-ink font-medium w-32 shrink-0">{alert.agent}</span>
                <span className="text-sm text-ink-secondary flex-1 truncate">{alert.description}</span>
                <span className="text-sm text-ink-muted font-mono w-16 text-right">{(alert.confidence * 100).toFixed(0)}%</span>
                <Icon
                  name="chevron-down"
                  size={16}
                  className={`text-ink-muted transition-transform duration-200 ${expanded === i ? 'rotate-0' : '-rotate-90'}`}
                />
              </div>
              ...
              >
                <div className="px-4 pb-4 pt-2 border-t border-rule space-y-3">
                  <div>
                    <p className="text-xs text-ink-muted font-medium uppercase tracking-wider mb-1.5">Evidence</p>
                    <ul className="space-y-1">
                      {alert.evidence.map((e, j) => (
                        <li key={j} className="text-sm text-ink-secondary flex items-start gap-2">
                          <Icon name="chevron-right" size={12} className="text-ink-muted mt-0.5 shrink-0" />
                          {e}
                        </li>
                      ))}
                    </ul>
                  ...
                  <div>
                    <p className="text-xs text-ink-muted font-medium uppercase tracking-wider mb-1.5">Recommended Action</p>
                    <p className="text-sm text-signal-cyan">{alert.recommended_action}</p>
                  </div>
                  {/* AI Explanation */}
                  <div className="pt-2 border-t border-rule/50">
                    {!explanations[i] ? (
                      canUseLLM ? (
                        <>
                          ...
                          {explaining === i && streamingText[i] && (
                            <pre className="mt-2 text-xs text-ink-secondary whitespace-pre-wrap font-mono bg-surface rounded-lg p-2 max-h-32 overflow-y-auto">
                              {streamingText[i]}
                      ...
                      <div className="space-y-2 animate-fadeIn">
                        ...
                        <p className="text-sm text-ink">{explanations[i].explanation}</p>
                        {explanations[i].recommended_actions?.length > 0 && (
                          <div>
                            <p className="text-xs text-ink-muted mb-1">AI Recommended Actions</p>
                            <ul className="space-y-1">
                              {explanations[i].recommended_actions.map((a, j) => (
                                <li key={j} className="text-sm text-signal-cyan flex items-start gap-2">
                                  <Icon name="chevron-right" size={12} className="text-signal-cyan mt-0.5 shrink-0" />
                                  {a}
                                </li>
                              ))}
                            </ul>
                  ...
                  <div className="flex gap-4 text-xs text-ink-muted">
                    ...
                    <RelativeTime timestamp={alert.timestamp} className="text-ink-muted" />
                  </div>
                </div>
              </div>
            </div>

...
def mock_detector() -> MagicMock:
    ...
    detector = MagicMock()
    detector.adaptive_baselines = {"agent-a": AgentAdaptiveBaseline(agent_name="agent-a")}
    return detector
...
class TestSelfHealingEngine:
    ...
    def test_apply_agent_block(self, mock_client: MagicMock, mock_policy_engine: MagicMock) -> None:
        engine = SelfHealingEngine(client=mock_client)
        action = {
            "type": "agent_block",
            "target": "agent-a",
    ...
    def test_auto_remediate_above_threshold(
        ...
        engine = SelfHealingEngine(
            client=mock_client, auto_apply=True, auto_apply_confidence_threshold=0.9
        )
        engine.pending_actions = [
            {
                "type": "threshold_adjustment",
                "target": "agent-a",
                "value": {"rate_multiplier": 4.0},
                "confidence": 0.95,
                "reversible": True,
            },
        ]
